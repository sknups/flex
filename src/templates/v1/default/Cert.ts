...
export class DefaultTemplate extends BrandTemplate<ItemDTO> {
    ...
    async renderTemplate(dto: ItemDTO, use: string): Promise<Canvas> {
        ...
        let images = await this.loadImages([
            `./static/backgrounds/cert.front.default.${dto.certVersion}.jpg`,
            `sku.${dto.certVersion}.designImage.${dto.stockKeepingUnitCode}.png`,
            `brand.${dto.certVersion}.logo.${dto.brandCode}.png`,
        ]);
        ...
        Y = Y + writeTextCentered(context, dto.stockKeepingUnitName.toLocaleUpperCase(), this.CENTER_X , Y);
        ...
        Y = Y + this.SPACE
        Y = Y + writeTextCentered(context, dto.thumbprint, this.CENTER_X, Y);
        ...
        if (dto.description.length > 250) { context.font = '22pt Jost'; } else { context.font = '24pt Jost'; }
        this.wrapTextCentered(context, dto.description, this.CENTER_X, Y , this.PRODUCT_DESCRIPTION_WIDTH , 45);
