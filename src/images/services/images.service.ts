...
export class ImagesService {
    ...
    legacyGenerateCanvas(fromCertificate: CertificateDTO, use: string, callback: Function) {
        ...
        Promise.allSettled([backgroundPromise, brandPromise, gamePromise, skuPromise]).then((images) => {
            ...
            this.legacyWrapText(context, fromCertificate.description, 325, 100, 500, 30);
            ...
            this.legacyWrapText(context, 'SOLD TO ' + fromCertificate.gamer_tag.toUpperCase() + ' FOR UNLIMITED USE IN ' + fromCertificate.platform.toUpperCase(), 325, 75, 500, 30);
            ...
            callback(false, canvas.toBuffer());
        });
    }

    // Will be replaced
    //@ts-ignore
    legacyWrapText(context, text, x, y, maxWidth, lineHeight) {
    ...
    // @ts-ignore
    legacyGetKeyByValue(object, value) {
